...
export function RecordedMaxExpBarChart() {
      ...
      if (keys.length > 0 && keys[keys.length-1].localeCompare(currentTime) <= 0) {
        ...
        setcummulativeMaxExpressions(
          currentLabels.flatMap(
            (x) => totalRI?.totalExpressions[x]?.maxExpression?.value
          )
        );
        setBarColors(
          currentLabels.flatMap((x) => {
            return {
              ...
              fearful: "#333",
            }[totalRI?.totalExpressions[x]?.maxExpression?.name];
          })
        );
        ...
        //   tempLabels.push(x);
        //   tempMaxExpression.push(totalRI?.totalExpressions[x]?.maxExpression?.value);
        //   tempBarColors.push(
        //     {
        //       angry: "rgba(240, 50, 50, .75)",
